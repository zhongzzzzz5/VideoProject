            ...
            if($data){
                if($data["email"]){
                    if($post["email"] == $data["email"]){//提交的邮箱于数据库邮箱相同
                        ...
                        $mail->Subject = 'www.5wpz.top【重置密码】' . time();
                        $mail->Body    = '<h1>您好！'.$addressee_name.'</h1><p>请点击链接以验证操作:'.SERVER_URL.__ROOT__.'/index.php/Admin/Public/emailVerify?request_token='.$request_token.'</p>' . date('Y-m-d H:i:s');
